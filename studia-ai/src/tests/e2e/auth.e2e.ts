...
test.describe("Authentication E2E", () => {
  ...
  test.describe("Sign In Flow", () => {
    ...
    test("should show error for invalid credentials", async ({ page }) => {
      ...
      await expect(
        page.locator("text=Invalid credentials|Error"),
      ).toBeVisible();
    });
  ...
  test.describe("Protected Routes", () => {
    test("should redirect to signin when accessing dashboard while logged out", async ({
      page,
    }) => {
    ...
    test("should allow access to dashboard when logged in", async ({
      page,
    }) => {
      // First, sign in
      await page.goto("/auth/signin");
  ...
  test.describe("Password Reset Flow", () => {
    ...
    test("should show success message even for non-existent email (anti-enumeration)", async ({
      page,
    }) => {
      await page.goto("/auth/forgot-password");
    ...
    test("should display reset password page with valid token", async ({
      ...
      await page.goto(
        `/auth/reset-password?token=${testToken}&email=test@example.com`,
      );
  ...
  test.describe("Rate Limiting", () => {
    test("should lock account after multiple failed attempts", async ({
      ...
      await expect(
        page.locator("text=locked|Invalid credentials"),
      ).toBeVisible();
    });
  });
});
...
test.describe("Navigation and UI", () => {
  test("should navigate from signin to signup", async ({ page }) => {
    ...
    const signupLink = page.locator(
      'a:has-text("Sign Up"), a:has-text("Register"), a[href*="signup"]',
    );
  ...
  test("should navigate from signup to signin", async ({ page }) => {
    ...
    const signinLink = page.locator(
      'a:has-text("Sign In"), a:has-text("Login"), a[href*="signin"]',
    );
