...
describe('M6 Study Engine - Business Logic', () => {
  ...
  describe('Question Type Handling', () => {
    it('should require selectedOptionId for multiple choice questions', () => {
      ...
      const requiresOption = questionType === 'multiple_choice' || questionType === 'true_false';
      expect(requiresOption).toBe(true);
      
      const hasOption = selectedOptionId !== null && selectedOptionId !== undefined;
    ...
    it('should require answerText for open-ended questions', () => {
      ...
      const requiresText = questionType === 'short_answer' || questionType === 'open_ended';
      expect(requiresText).toBe(true);
      
      const hasText = answerText !== null && answerText !== undefined && answerText.length > 0;
